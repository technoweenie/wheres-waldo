...
function WheresWaldo(redis, prefix) {
  ...
  this.track = function(user, location, ttl) {
    ...
    promise.subPromises = [
      this.redis.set(this.prefix + ":" + user, location).addCallback(function() {
        promise.subPromises.shift();
        if(promise.subPromises.length == 0)
          promise.emitSuccess();
      }),
      this.redis.set(this.prefix + ":" + location + ":" + user, user).addCallback(function() {
        promise.subPromises.shift();
        if(promise.subPromises.length == 0)
          promise.emitSuccess();
      })
  ...
  this.locate = function(user) {
    return this.redis.get(prefix + ":" + user)
  }
